...
public class RatingController {
    ...
    public ApiResponseNormal<Void> rateArticle(@PathVariable Integer articleId, @RequestBody RatingRequest ratingRequest) {
        String result = ratingService.rateArticle(articleId, ratingRequest.getExpertId(), ratingRequest.getInnovationScore(), ratingRequest.getDisruptionScore(), ratingRequest.getFrontierScore(), ratingRequest.getIndustryImpactScore());
    ...
    public ApiResponseNormal<List<ArticleRating>> getArticleRatingHistory(@PathVariable Integer articleId) {
        List<ArticleRating> ratingHistory = ratingService.getArticleRatingHistory(articleId);
        return new ApiResponseNormal<>(200, ratingHistory, "获取历史评分成功");
    }
}
